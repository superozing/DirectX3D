...
FOCUS_STATE CImGuiMgr::GetFocus_debug()
{
	...
	// 현재 포커싱이 뷰포트일 경우
	if (GetFocus() == CEngine::GetInst()->GetMainWind() && IsViewportFocused() && !CRenderMgr::GetInst()->IsEscape())
	{
...
void CImGuiMgr::create_ui()
{
	...
	pUI = new Inspector;
	AddUI(pUI->GetID(), pUI);
	CLevelMgr::GetInst()->RegisterClientFunction(Client_Function_Type::CIMGUIMGR_ENTER,
												 std::bind(&CImGuiMgr::enter, this));
