...
void LandScapeUI::render_update()
{
	...
	static string strLandScapeMode =
		ToString(magic_enum::enum_name(GetTargetObject()->LandScape()->GetLandScapeMode()));
	...
	ImGui::Image(GetTargetObject()->LandScape()->GetHeightMapTex()->GetSRV().Get(),
				 ImVec2(ImGui::GetWindowSize().x, 150.f), uv_min, uv_max, tint_col, border_col);
	...
	if (ImGui::BeginCombo("##ComboHeightMap", strHeightMap.c_str()))
	{
		for (int n = 0; n < m_vecHeightTextureKey.size(); n++)
		{
			...
			if (ImGui::Selectable(m_vecHeightTextureKey[n].c_str(), is_selected))
			{
				...
				GetTargetObject()->LandScape()->SetHeightMapTex(
	...
	ImGui::Image(GetTargetObject()->LandScape()->GetBrushTex()->GetSRV().Get(), ImVec2(ImGui::GetWindowSize().x, 150.f),
	...
	if (ImGui::BeginCombo("##ComboBrushTex", strBrush.c_str()))
	{
		for (int n = 0; n < m_vecBrushTextureKey.size(); n++)
		{
			...
			if (ImGui::Selectable(m_vecBrushTextureKey[n].c_str(), is_selected))
			{
				...
				GetTargetObject()->LandScape()->SetBrushTex(
					CAssetMgr::GetInst()->Load<CTexture>(ParticleKey, ParticleKey));
			}
		}
		ImGui::EndCombo();
	...
	vecBrushScale.x = fBrushSize[0];
	vecBrushScale.y = fBrushSize[1];
	GetTargetObject()->LandScape()->SetBrushScale(vecBrushScale);

	ImGui::SameLine();

	ImGui::Text("Leveling Up");
	ImGui::SameLine();
