...
{
	if (!TitleCollapse("ParticleSystem"))
		return;
	...
	if (ImGui::BeginCombo("##ComboParticle", strParticleTex.c_str()))
	{
		for (int n = 0; n < m_vecParticleKey.size(); n++)
		{
			...
			if (ImGui::Selectable(m_vecParticleKey[n].c_str(), is_selected))
			{
				...
				GetTargetObject()->ParticleSystem()->SetParticleTex(
					CAssetMgr::GetInst()->Load<CTexture>(ParticleKey, ParticleKey));
			}
		}
		ImGui::EndCombo();
	}
	...
	ImGui::Image(GetTargetObject()->ParticleSystem()->GetParticleTex()->GetSRV().Get(), ImVec2(UIsize.x - 10.f, 150.f),
	...
	ImGui::SameLine();
	ImGui::InputText("##ParticleTexName", (char*)strTextureName.c_str(), strTextureName.length(),
	...
	if (NewModule.arrModuleCheck[(UINT)PARTICLE_MODULE::ADD_VELOCITY] == 1)
	{
		...
		ImGui::SameLine();
		ImGui::DragFloat4("##FixedDirection", &NewModule.FixedDirection.x, 0.1, -1, 1);
	...
	StaticButton(string("Animation Module"), STATIC_BTN_TYPE::SUBTITLE);

	ImGui::SameLine();

	ImGui::PushStyleColor(ImGuiCol_Button, m_ModuleButtonColor[0]);
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, m_ModuleButtonColor[1]);
	if (1 == NewModule.arrModuleCheck[(UINT)PARTICLE_MODULE::ANIMATION])
	{
		if (ImGui::Button("On##Animation"))
		{
			NewModule.arrModuleCheck[(UINT)PARTICLE_MODULE::ANIMATION] = 0;
		}
	}
	else
	{
		if (ImGui::Button("Off##Animation"))
		{
			NewModule.arrModuleCheck[(UINT)PARTICLE_MODULE::ANIMATION] = 1;
		}
	}
	ImGui::PopStyleColor(2);
	if (NewModule.arrModuleCheck[(UINT)PARTICLE_MODULE::ANIMATION] == 1)
	{
		ImGui::Text("TileX");
		ImGui::SameLine();
		ImGui::DragInt("##TileX", &NewModule.TileX, 1.f, 0, 1000);
		ImGui::Text("TileY");
		ImGui::SameLine();
		ImGui::DragInt("##TileY", &NewModule.TileY, 1.f, 0, 1000);
	}
