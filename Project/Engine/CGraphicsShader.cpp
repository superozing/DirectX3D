...
int CGraphicsShader::CreateVertexShader(const wstring& _strRelativePath, const string& _strFuncName)
{
	...
	// Layout 생성
	DEVICE->CreateInputLayout(arrElement, 21
	...
	HRESULT hr = D3DCompileFromFile(strFilePath.c_str(),
									...
									(_strFuncName + "_Inst").c_str(), "vs_5_0",
									0, 0, m_VSInstBlob.GetAddressOf(), m_ErrBlob.GetAddressOf());

	if (SUCCEEDED(hr))
	{
		// 컴파일 된 코드로 Vertex Instancing Shader 객체 만들기
		DEVICE->CreateVertexShader(m_VSInstBlob->GetBufferPointer(), m_VSInstBlob->GetBufferSize(),
									nullptr, m_VSInst.GetAddressOf());

		DEVICE->CreateInputLayout(arrElement, 21, m_VSInstBlob->GetBufferPointer(),
									m_VSInstBlob->GetBufferSize(), m_LayoutInst.GetAddressOf());
	}
	else
	{
		return E_FAIL;
