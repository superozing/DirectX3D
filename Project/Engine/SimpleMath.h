...
struct Vector2 : public XMFLOAT2
{
	...
	XM_CONSTEXPR explicit Vector2(float x)
		: XMFLOAT2(x, x)
	{
	}
	XM_CONSTEXPR Vector2(float _x, float _y)
		: XMFLOAT2(_x, _y)
	{
	}
	XM_CONSTEXPR Vector2(int _x, int _y)
		: XMFLOAT2((float)_x, (float)_y)
	{
	}
	XM_CONSTEXPR Vector2(UINT _x, UINT _y)
		: XMFLOAT2((float)_x, (float)_y)
	{
	}
...
struct Vector3 : public XMFLOAT3
{
	...
	XM_CONSTEXPR explicit Vector3(float x)
		: XMFLOAT3(x, x, x)
	{
	}
	XM_CONSTEXPR Vector3(float _x, float _y, float _z)
		: XMFLOAT3(_x, _y, _z)
	{
	}
	...
	Vector3(const Vector2& V)
		: XMFLOAT3(V.x, V.y, 0.f)
	{
	}
	Vector3(const Vector2& V, float f)
		: XMFLOAT3(V.x, V.y, f)
	{
	}
	Vector3(float f, const Vector2& V)
		: XMFLOAT3(f, V.x, V.y)
	{
	}
	...
	float& operator[](int _iIdx) { return *((float*)this + _iIdx); }
	...
	Vector3 RotateAround(const Vector3& axis, float angle) const
	{
		float c = cos(angle);
		float s = sin(angle);

		return Vector3((x * (axis.x * axis.x * (1 - c) + c)) + (y * (axis.x * axis.y * (1 - c) - axis.z * s)) +
						   (z * (axis.x * axis.z * (1 - c) + axis.y * s)),
					   (x * (axis.x * axis.y * (1 - c) + axis.z * s)) + (y * (axis.y * axis.y * (1 - c) + c)) +
						   (z * (axis.y * axis.z * (1 - c) - axis.x * s)),
					   (x * (axis.x * axis.z * (1 - c) - axis.y * s)) + (y * (axis.y * axis.z * (1 - c) + axis.x * s)) +
...
struct Vector4 : public XMFLOAT4
{
	...
	XM_CONSTEXPR explicit Vector4(float x)
		: XMFLOAT4(x, x, x, x)
	{
	}
	XM_CONSTEXPR Vector4(float _x, float _y, float _z, float _w)
	...
	Vector4(const Vector2& V)
		: XMFLOAT4(V.x, V.y, 0.f, 0.f)
	{
	}
	Vector4(const Vector2& V1, const Vector2& V2)
		: XMFLOAT4(V1.x, V1.y, V2.x, V2.y)
	{
	}
	Vector4(const Vector2& V, float f1, float f2)
		: XMFLOAT4(V.x, V.y, f1, f2)
	{
	}
	Vector4(float f1, const Vector2& V, float f2)
		: XMFLOAT4(f1, V.x, V.y, f2)
	{
	}
	Vector4(float f1, float f2, const Vector2& V)
		: XMFLOAT4(f1, f2, V.x, V.y)
	{
	}
	Vector4(const Vector2& V, float f)
		: XMFLOAT4(V.x, V.y, f, 0.f)
	{
	}
	Vector4(float f, const Vector2& V)
		: XMFLOAT4(f, V.x, V.y, 0.f)
	{
	}

	// Vec4(Vec3)
	Vector4(const Vector3& V)
		: XMFLOAT4(V.x, V.y, V.z, 0.f)
	{
	}
	Vector4(float f, const Vector3& V)
		: XMFLOAT4(f, V.x, V.y, V.z)
	{
	}
...
struct Matrix : public XMFLOAT4X4
{
	...
	XM_CONSTEXPR Matrix(float m00, float m01, float m02, float m03, float m10, float m11, float m12, float m13,
...
struct Plane : public XMFLOAT4
{
	...
	XM_CONSTEXPR Plane(float _x, float _y, float _z, float _w)
		: XMFLOAT4(_x, _y, _z, _w)
	{
	}
...
struct Quaternion : public XMFLOAT4
{
	...
	XM_CONSTEXPR Quaternion(float _x, float _y, float _z, float _w)
		: XMFLOAT4(_x, _y, _z, _w)
	{
	}
