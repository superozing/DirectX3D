...
public:
	void Lerp(Vec3 _pos, bool _bMoveRot = false, Vec3 _rot = Vec3(), bool _bMoveScale = false,
			  Vec3 _scale = Vec3(), float _time = 0.f);
...
public:
	...
	Vec3 GetRelativeRotation() const
	{
	...
	void SetAbsolute(bool _bAbsolute)
	{
		m_bAbsolute = _bAbsolute;
	}
	...
	void SetDynamicObject(bool _Dynamic)
	{
		m_IsDynamic = _Dynamic;
	}
	bool IsDynamic()
	{
		return m_IsDynamic;
	...
	Vec3 GetLocalDir(DIR_TYPE _type) const
	{
		return m_arrLocalDir[(UINT)_type];
	}
	Vec3 GetWorldDir(DIR_TYPE _type) const
	{
		return m_arrWorldDir[(UINT)_type];
	}

	Matrix GetWorldViewMatrix()
	{
		return g_Transform.matWV;
	}
	Vec3 GetWorldViewPos() const
	{
		return g_Transform.matWV.Translation();
	}
