...
public:
	...
	void AddScalarParam(SCALAR_PARAM _Param, const string& _Desc, float _min = 0.f, float _Max = 0.f,
						bool _View = false, const string& _Tooltip = {})
	{
		m_ScalarParam[(UINT)_Param] = tScalarParam{_Param, _Desc, _min, _Max, _View, _Tooltip, true};
	}
	void AddTexParam(TEX_PARAM _Param, const string& _Desc)
	{
		m_TexParam[(UINT)_Param] = tTexParam{_Param, _Desc, true};
	}
